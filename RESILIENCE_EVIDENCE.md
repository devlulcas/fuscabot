...
| B6 Drizzle/SQL/authorization | Drizzle owns workspace bootstrap, auth/session, tags, and rate-limit operations. Parameter/adversarial tests cover resource search and bulk ownership; delivery retry lookup and tag update are workspace-scoped | Partial | Medium: resource, channel setup, enrichment CTEs, and durable delivery repositories still use parameterized legacy `pg` queries. No user value becomes SQL syntax, but these should be migrated repository-by-repository without weakening atomic state transitions |
...
- Remaining repository SQL listed under B6 is parameterized and has focused
  authorization/state-transition tests, but is transitional rather than an
