...
- Partial successes remain successful and are reported per destination.
  Website and Discord failures have targeted retry actions.
...
- Website visibility is explicit and independent of Discord delivery state.
  "Remove from public site" unpublishes without deleting Discord messages or
  the saved resource.
...
- A public slug is created on first publication from a normalized title plus
  an eight-character random suffix. It is immutable while the resource exists.
...
Discord messages retain the reviewed plain-Markdown format and link directly
to the safe source URL rather than the archive page. Personal notes may remain
in that Discord snapshot but are never part of the website projection.
...
Remove `archived_at` only after migrating archived records back to Inbox.
...
- invalid query shapes return a localized `400`, while an unknown tag returns
  an empty `200` result.
...
semantic landmarks, a skip link, visible keyboard focus, reduced-motion
support, accessible pagination, localized link labels, self-canonical URLs,
`hreflang` alternates, and Open Graph metadata.
...
- Preserve request IDs and structured logs without query values, titles,
  notes, URLs, tokens, or private resource data.
...
- Key public limits by the `Deno.serve` remote address and hash keys through
  the existing PostgreSQL limiter: 120 page requests per minute and 30 search
  requests per minute. Health and immutable assets are exempt. Return `429`
  with `Retry-After`.
...
Analytics is absent in development and tests unless configured. Production
uses:
...
unpublished data. Track source-link clicks as `outbound-link` with source
domain as the only custom event property and no inline event handler.
...
- authenticated publication, ownership, eligibility, partial outcomes,
  targeted retries, idempotency, additional Discord destinations, unpublish,
  republish, deletion, and visibility filtering;
...
- both locales, negotiation, search ranking, tags, pagination, escaping,
  metadata, sitemap, robots, ETags, caching, CSP, rate limiting, and generic
  404 behavior;
- Umami is absent without configuration and emits the approved script,
  privacy attributes, and outbound event markup when configured;
